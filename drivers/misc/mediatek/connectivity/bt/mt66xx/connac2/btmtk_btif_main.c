		...
		case PSM_ST_NORMAL_TR:
			...
			if (bdev->bt_state == FUNC_ON && cmd_list_isempty() &&
			   psm->sleep_flag && !psm->force_on) {
				// wait 1ms if btif tx is not finish yet
				if (g_btif_id && mtk_btif_is_tx_complete(g_btif_id) <= 0) {
					wait_tx_complete_done++;
					msleep(1);
					break;
				}
				if(wait_tx_complete_done > 0) {
					BTMTK_WARN("(PSM_ST_NORMAL_TR) wait btif tx done for i[%d] times", wait_tx_complete_done);
					wait_tx_complete_done = 0;
				}                          
